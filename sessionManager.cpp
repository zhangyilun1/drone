...
void SessionManager::HeartBeats()
{
    while (true)
    {
        ...
        for (auto it = m_sessMap.begin(); it != m_sessMap.end(); it++)
        {
            ...
            max_fd = std::max(max_fd, current_fd);
        }
        struct timeval timeout;
        timeout.tv_sec = 14; // 设置超时时间为2秒
        timeout.tv_usec = 0;
        int activity = select(max_fd + 1, &read_fds, NULL, NULL, &timeout);
        TRACE(" num of activity session : " + to_string(activity));
        if (activity < 0)
        {
            TRACE("select error: " + std::string(strerror(errno)));
            continue;
        }
        else if (activity == 0)
        {
            TRACE("Timeout - Connection interrupted. " + std::string(strerror(errno)));
            break;
        }
        else
        {
            ...
            {
                int current_fd = it->second->socket();
                TRACE(" current_fd : " + to_string(current_fd));
    ...
    map<int64_t, Session *> SessionManager::ValidateSession()
    {
        ...
        for (size_t i = 0; i < delVec.size(); i++)
        {
            ...
            TRACE("delete session form m_sessMap : " + to_string(sessionToDelete->socket()));
            ...
            // 删除 Session 对象
            close(sessionToDelete->socket());
            // sessionToDelete->socket() = -1;
