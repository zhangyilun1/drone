...
std::vector<CommuteTask> CPointToLine::route_to_osdkroute(CommissionType2Post tempcommissionType2Post) {
	std::vector<CommuteTask> vector_commute_task;
	QString temp_poi = "";
	for (int i = 0; i < tempcommissionType2Post.stc_fplist.size(); i++)
	{
		CommuteTask temp_commuteTask;
		CDatumPoint m_CDatumPoint;

		//转化存在问题
		for (int j = 0; j < tempcommissionType2Post.stc_fplist[i].target_class_id.size(); j++)
		{
			temp_commuteTask.targets.push_back((TargetType)tempcommissionType2Post.stc_fplist[i].target_class_id[j]);
		...
		temp_commuteTask.longitude = tempcommissionType2Post.stc_fplist[i].stc_gps.stc_lon;
		temp_commuteTask.latitude = tempcommissionType2Post.stc_fplist[i].stc_gps.stc_lat;
		temp_commuteTask.relativeHeight = tempcommissionType2Post.stc_fplist[i].stc_gps.stc_alt;

		temp_commuteTask.XJ.poi.yaw = tempcommissionType2Post.stc_fplist[i].stc_yaw;
		temp_commuteTask.XJ.poi.gimbal_pitch = tempcommissionType2Post.stc_fplist[i].stc_gimbalPitch;
		...
		Coordinate aaaa;
		aaaa.stc_x = 30 * sin(ang2rad(temp_commuteTask.XJ.poi.yaw));
		aaaa.stc_y = 30 * cos(ang2rad(temp_commuteTask.XJ.poi.yaw));
		aaaa.stc_z = 30 * tan(ang2rad(temp_commuteTask.XJ.poi.gimbal_pitch));
		...
		if (tempcommissionType2Post.stc_fplist[i].stc_gimbalPitch == 3) {
			...
			aaaa.stc_x = 300 * sin(ang2rad(temp_commuteTask.XJ.poi.yaw));
			aaaa.stc_y = 300 * cos(ang2rad(temp_commuteTask.XJ.poi.yaw));
			aaaa.stc_z = 300 * sin(ang2rad(temp_commuteTask.XJ.poi.gimbal_pitch));
		...
		if (tempcommissionType2Post.stc_fplist[i].stc_gimbalPitch == -90) {
			...
			aaaa.stc_x = 3000 * sin(ang2rad(temp_commuteTask.XJ.poi.yaw));
			aaaa.stc_y = 3000 * cos(ang2rad(temp_commuteTask.XJ.poi.yaw));
			aaaa.stc_z = sin(ang2rad(temp_commuteTask.XJ.poi.gimbal_pitch));
		...
		temp_commuteTask.XJ.way_point.POI_Y = m_CDatumPoint.longitudeAndLatitudeToDatumPoint(calculate_interest_point).stc_y;
		////longitude can calculate x
		temp_commuteTask.XJ.way_point.POI_X = m_CDatumPoint.longitudeAndLatitudeToDatumPoint(calculate_interest_point).stc_x;
		////relativeHeight can calculate z
		temp_commuteTask.XJ.way_point.POI_Z = m_CDatumPoint.longitudeAndLatitudeToDatumPoint(calculate_interest_point).stc_z;
		if (tempcommissionType2Post.stc_fplist[i].stc_shootPhoto == 0) {
			temp_commuteTask.XJ.way_point.POI_Z = 1;
		}
