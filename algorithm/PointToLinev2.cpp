#include "PointToLinev2.h"

using namespace std;

PointToLineV2::PointToLineV2()
{
...
double PointToLineV2::ang2rad(int in) {
	double radius = (double)in / 180 * 3.1415926;
	return radius;
}

float PointToLineV2::angplus(float angle_1, float angle_2) {
...
int PointToLineV2::rad2ang(double in) {
	double degree = in / 3.1415926 * 180;
	return degree;
}

vector<double> PointToLineV2::matrixrotate2d(vector<double> in_point, int theta)
{
	double x = in_point[0];
	double y = in_point[1];
...
vector<double> PointToLineV2::get_unit_directon2d(double x1, double y1, double x2, double y2)
{
	double dx = x2 - x1;
	double dy = y2 - y1;
...
vector<double> PointToLineV2::get_unit_directon3d(double x1, double y1, double z1, double x2, double y2, double z2)
{
	double dx = x2 - x1;
	double dy = y2 - y1;
	double dz = z2 - z1;
...
double PointToLineV2::get_distance2d(double x1, double y1, double x2, double y2)
{
	double dx = x2 - x1;
	double dy = y2 - y1;
	double square = dx * dx + dy * dy;
...
double PointToLineV2::get_distance3d(double x1, double y1, double z1, double x2, double y2, double z2)
{
	double dx = x2 - x1;
	double dy = y2 - y1;
	double dz = z2 - z1;
...
int PointToLineV2::get_angle_tonorth(double  x2, double  y2)
{
	double z1 = 0;
	double z2 = 0;
	double x1 = 0;
...
std::vector<WaypointInformation> PointToLineV2::pointToLine_U_2_V2(RecPoint input)
{
	...
	if (pow(initial_line_prev_point.stc_x, 2) + pow(initial_line_prev_point.stc_y, 2) <= 1)
	{
		...
		point_0.POI_Z = initial_line_up_point.stc_z - point_0.stc_z - 20;
	...
	else
	{
		...
		if (ChannelInspection == 1) {
			...
			for (int iii = 3; iii > 1; iii--) {
				...
				point_0.stc_shootPhoto = 1;
				point_0.zoom_Magnification = 1;
				std::vector<uint8_t> temp_target_class_id_tep;
	...
	for (int m = 0; m <= layer_num; m++)
	{
		...
		point_1.orient_to_tower = 0;
		//point_1.Number_of_layers_of_current_waypoint = 0.5 + m;//float->int
		point_1.Number_of_layers_of_current_waypoint = m;
		point_1.POI_X = initial_line_up_point.stc_x - point_1.stc_x;
		point_1.POI_Y = initial_line_up_point.stc_y - point_1.stc_y;
		point_1.POI_Z = 0;
		point_1.waypointType = 1;
	...
	if (ChannelInspection == 1) {
		...
		if (pow(initial_line_prev_point.stc_x, 2) + pow(initial_line_prev_point.stc_y, 2) <= 1)
		{
			...
			point_0.POI_Z = d_right_prev_perpendicular[2] * longlook_whole_tower_distance - point_0.stc_z - 20;
		}
		else {
			...
			point_0.POI_Z = initial_line_prev_point.stc_z - point_0.stc_z - 20;
		}
		...
		point_0.target_class_id = temp_target_class_id;
		point_0.orient_to_tower = 0;
		//point_0.Number_of_layers_of_current_waypoint = 0.5 + layer_num;//float->int
		point_0.Number_of_layers_of_current_waypoint = layer_num;
	...
	for (int m = layer_num; m >= 0; m--)
	{
		...
		point_1.target_class_id = temp_target_class_id;
		point_1.orient_to_tower = 0;
		//point_1.Number_of_layers_of_current_waypoint = layer_num +0.5 - m;//float->int
		point_1.Number_of_layers_of_current_waypoint = layer_num  - m;
		point_1.POI_X = initial_line_up_point.stc_x - point_1.stc_x;
		point_1.POI_Y = initial_line_up_point.stc_y - point_1.stc_y;
		point_1.POI_Z = 0;
		point_1.waypointType = 1;
	...
	if (ChannelInspection == 1) {
		...
		if (pow(initial_line_next_point.stc_x, 2) + pow(initial_line_next_point.stc_y, 2) <= 1)
		{
			...
			point_0.POI_Z = initial_line_next_point.stc_z - point_0.stc_z - 20;
		}
...
/////////////////////////////U型精简两次
std::vector<WaypointInformation> PointToLineV2::pointToLine_U_S2_V2(RecPoint input)
{
	...
	if (ChannelInspection == 1) {
		...
		//point_0.Number_of_layers_of_current_waypoint = layer_num + 0.5;//float->int
		point_0.Number_of_layers_of_current_waypoint = layer_num;
		point_0.POI_X = initial_line_up_point.stc_x - point_0.stc_x;
		point_0.POI_Y = initial_line_up_point.stc_y - point_0.stc_y;
		point_0.POI_Z = initial_line_up_point.stc_z - point_0.stc_z - 20;
...
////////////////////////////倒U一侧单次
std::vector<WaypointInformation> PointToLineV2::pointToLine_OU_1_V2(RecPoint input)
{
	vector<WaypointInformation> line_out;
	line_out.clear();
	DatumPoint initial_line_up_point = input.rec_pt[0];
	...
	*/
	...
	else
	{
		...
		point_0.Number_of_layers_of_current_waypoint = -1;
		point_0.POI_X = (longlook_whole_tower_distance + 10) * unit_next_directon[0] - point_0.stc_x;
		point_0.POI_Y = (longlook_whole_tower_distance + 10) * unit_next_directon[0] - point_0.stc_y;
		point_0.POI_Z = 0;
		point_0.waypointType = 1;
		line_out.push_back(point_0);
	}
...
std::vector<WaypointInformation> PointToLineV2::route_to_osdkroute_V2(CommissionType2Post tempcommissionType2Post) {
